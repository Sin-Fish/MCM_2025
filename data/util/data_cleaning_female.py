...
class data_cleaner:
    ...
    def _process_column_value(self, index, col_name, value):
        ...
        elif col_name == '孕妇BMI' and pd.isna(value):
            # 处理BMI列
            self._calculate_bmi_for_row(index)
        # 可以继续添加其他列的处理逻辑
    
    def _calculate_bmi_for_row(self, index):
        """
        为特定行计算BMI值
        param:
        index: 行索引
        ...
        # 检查身高和体重是否都存在且为数值
        if pd.notna(height) and pd.notna(weight) and isinstance(height, (int, float)) and isinstance(weight, (int, float)):
            # 将身高从厘米转换为米
            height_m = height / 100
            # 计算BMI = 体重(kg) / 身高(m)^2
            bmi = weight / (height_m ** 2)
            self.data.at[index, '孕妇BMI'] = round(bmi, 2)
...
if __name__ == "__main__":
    ...
    config = {
        "file_path": os.path.join(project_dir, "data", "女胎检测数据.xlsx"),  
        "save_path": os.path.join(project_dir, "data", "cleaned_data_female.csv"),  # 改为CSV格式
    }
    ...
    heatmap_before_save_path = os.path.join(project_dir, "data", "female_missing_values_heatmap_before.png")
    ...
    # 生成缺失值热力图，可视化数据质量
    heatmap_save_path = os.path.join(project_dir, "data", "female_missing_values_heatmap.png")
