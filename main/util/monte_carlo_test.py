...
class MonteCarloFramework:
    ...
    def __init__(self, model=None, cluster_model=None, feature_columns=None):
        """
        初始化蒙特卡洛框架
        ...
        self.feature_columns = feature_columns or ['孕妇BMI', 'Y染色体浓度']
        
    def add_noise_to_data(self, data, sigma=0.1):
        """
        向数据添加高斯噪声
    ...
    def prepare_pregnancy_data_with_noise(self, original_data, sigma_y=0.1, sigma_time=0.1, check_time='检测孕周'):
        """
        ...
        noisy_y_concentration = self.add_noise_to_data(original_data['Y染色体浓度'].values, sigma_y)
        # 确保噪声后的Y染色体浓度不为负数
        ...
        if sigma_time > 0:
            noisy_time = self.add_noise_to_data(original_data[check_time].values, sigma_time)
            # 确保噪声后的时间不为负数
    ...
    def monte_carlo_simulation(self, original_data, n_simulations=100, sigma_y=0.1, sigma_time=0.1, 
        ...
        if self.cluster_model is None:
            self.cluster_model = KMeansCluster(n_clusters=4)
        self.cluster_model.train(X_original)
        ...
        for i in range(n_simulations):
            ...
            # 生成扰动数据集
            perturbed_data = self.prepare_pregnancy_data_with_noise(original_data, sigma_y, sigma_time, check_time)
            ...
            # 对数据进行聚类预测
            try:
                km_clusters = self.cluster_model.predict(km_X)
    ...
    def days_to_weeks_days(self, days):
        """将天数转换为周数和天数的格式"""
        weeks = int(days // 7)
        remaining_days = int(days % 7)
        return weeks, remaining_days

    def format_gestational_age(self, days):
        """格式化孕周显示"""
        weeks, days = self.days_to_weeks_days(days)
        if days == 0:
            return f"{weeks}周"
        else:
            return f"{weeks}周+{days}天"

    def prepare_baseline_data(self, original_data, check_time='检测孕周'):
        """
        准备未扰动的基准数据
        ...
        return baseline_data_clean, self.cluster_model

    def compare_with_baseline(self, original_data, simulation_results, percentiles=[50, 90], check_time='检测孕周'):
        """
        将模拟结果与未扰动的基准数据进行比较
        ...
        """
        # 准备基准数据
        baseline_data, kmeans = self.prepare_baseline_data(original_data, check_time)
        ...
        for p in percentiles:
            ...
            if p in simulation_results['stats']:
                ...
                print(f"{p}%\t{self.format_gestational_age(baseline_value)} ({baseline_value:.2f})\t"
                      f"{self.format_gestational_age(sim_mean)} ({sim_mean:.2f})\t"
                      f"{diff:.2f}\t\t{cv:.4f}")
                
                comparison_results[p] = {
                    'baseline_value': baseline_value,
                    'baseline_formatted': self.format_gestational_age(baseline_value),
                    'simulation_mean': sim_mean,
                    'simulation_formatted': self.format_gestational_age(sim_mean),
                    'difference': diff,
    ...
    def visualize_simulation_results(self, simulation_results, sigma_y, sigma_time, percentiles=[50, 90]):
        """
        ...
        for i, p in enumerate(percentiles):
            if p in simulation_results['percentile_times']:
                ...
                axes[i].axvline(mean_time, color='red', linestyle='--', linewidth=2, 
                               label=f'均值: {self.format_gestational_age(mean_time)} ({mean_time:.2f}天)')
                axes[i].legend()
        
        plt.tight_layout()
        plt.show()

    def sensitivity_analysis(self, results_dict, percentiles=[50, 90]):
        """
            ...
            if y_only_results:
                ...
                if y_means:
                    ...
                    for i, sigma in enumerate(y_sigmas):
                        ...
                        print(f"{sigma}\t{self.format_gestational_age(y_means[i])} ({y_means[i]:.2f})\t{diff:.2f}")
            ...
            if time_only_results:
                ...
                if time_means:
                    ...
                    for i, sigma in enumerate(time_sigmas):
                        ...
                        print(f"{sigma}\t{self.format_gestational_age(time_means[i])} ({time_means[i]:.2f})\t{diff:.2f}")
    ...
    def run_sensitivity_analysis(self, original_data, sigma_y_values=[0.05, 0.1], sigma_time_values=[2.0, 5.0], 
        ...
        for sigma_y in sigma_y_values:
            print(f"\n分析σ_y={sigma_y}的情况:")
            result = self.monte_carlo_simulation(original_data, n_simulations, sigma_y, 0, check_time, percentiles)
            if result:
                results[f'y_{sigma_y}'] = result
                # 与基准数据比较
                comparison = self.compare_with_baseline(original_data, result, percentiles, check_time)
                result['comparison'] = comparison
                
                # 可视化结果
                # if visualize:
                #     self.visualize_simulation_results(result, sigma_y, 0, percentiles)
        
        # 测试不同的时间噪声水平（Y浓度噪声为0）
        print("\n--- 测试检测时间噪声对结果的影响 (Y浓度噪声σ_y=0) ---")
        for sigma_time in sigma_time_values:
            print(f"\n分析σ_time={sigma_time}的情况:")
            result = self.monte_carlo_simulation(original_data, n_simulations, 0, sigma_time, check_time, percentiles)
            if result:
                ...
                # # 可视化结果
                # if visualize:
                #     self.visualize_simulation_results(result, 0, sigma_time, percentiles)
        ...
        for sigma_y in sigma_y_values:
            for sigma_time in sigma_time_values:
                ...
                result = self.monte_carlo_simulation(original_data, n_simulations, sigma_y, sigma_time, check_time, percentiles)
                if result:
                    ...
                    if visualize and sigma_y == sigma_y_values[len(sigma_y_values)-1] and sigma_time == sigma_time_values[len(sigma_time_values)-1]:
                        self.visualize_simulation_results(result, sigma_y, sigma_time, percentiles)
        
        # 执行敏感性分析
        self.sensitivity_analysis(results, percentiles)
        ...
        self.save_results_to_json(comparison_only_results)
